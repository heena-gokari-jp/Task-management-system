...
def task_list(request):
    ...
    try:
        ...
        for task in tasks_list:
            task_dict = model_to_dict(task,exclude=['categories'])
            category_names = [category_dict.get(cat.id, '') for cat in task.categories.all()]
            task_dict['category_names'] = ", ".join(filter(None, category_names))
        ...
        return render(request, 'tasks/task_list_new.html', context)
    except Exception as e:
        messages.error(request, f"An error occurred: {str(e)}")
        return render(request, 'tasks/task_list.html', {'tasks': [], 'categories': []})
